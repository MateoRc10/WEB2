...
export class CanchasListComponent implements OnInit {
  ...
  // ============================================================
  reservarCancha(canchaId: number): void {
    // Creamos una reserva para MAÑANA de 8 AM a 10 AM como demostración
    // En producción, aquí habría un selector de fecha y hora
    const manana = new Date();
    manana.setDate(manana.getDate() + 1);

    const inicio = new Date(manana);
    inicio.setHours(8, 0, 0, 0); // 8:00:00 AM exacto

    const fin = new Date(manana);
    fin.setHours(10, 0, 0, 0); // 10:00:00 AM exacto

    const nuevaReserva: ReservaCreate = {
      canchaId: canchaId,
      // .toISOString() convierte la fecha a formato ISO 8601: "2025-01-15T13:00:00.000Z"
      // Este es el formato estándar que .NET espera recibir para los DateTime
