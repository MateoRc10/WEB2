...
namespace ReservasWEB2.Controllers
{
    ...
    public class ReservasController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateReserva([FromBody] ReservaCreateDTO model)
        {
            ...
            // Validamos que la reserva no sea en el pasado (con un margen de 5 minutos)
            if (model.FechaInicio < DateTime.UtcNow.AddMinutes(-5))
            {
                return BadRequest(new { mensaje = "La fecha y hora de la reserva no pueden ser en el pasado." });
            }
